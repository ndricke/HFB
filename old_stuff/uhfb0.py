...
class UHFB(object):
    def __init__(s,h,V,m,G=None,tol=10**-7):
        s.h = h
        s.V = V
        s.m = m
        s.n = h.shape[0]
        ...
        s.G = G #Generalized density matrix
        s.F = uhf.UHF.genFock(s.G[:n,:n],s.h,s.V) 
        s.Delta = self.genDelta(s.G[:n,n:],s.V)
...
#Figure out two potentials that bracket the potential that must be applied for desired population
    ...
    def _do_hfb(s):
        ...
        while not converged:
            ...
            s.F = uhf.UHF.genFock(G_new[:s.n,:s.n],s.h,s.V) #is HFB the same as HF for this part?
...
#        s.E = np.trace(np.dot(s.G[:n,:n],s.H[:n,:n]+s.h-0.*s.mu*np.eye(s.n))-s.G[:n,n:].dot(s.H[:s.n,s.n:]))/s.n
    ...
    def genDelta(ki,V):
        ...
        for i,j,k,l in itertools.product(ri,ri,ri,ri):
            D[i,j] += 0.5*(V[i,j,k,l] - V[i,j,l,k])*ki[k,l]
        return D
